...
import com.bangalore.barcamp.UpdatesFetcherIntentService;
...
public class MainFragmentActivity extends BCBFragmentActionbarActivity {
	...
	protected void onNewIntent(Intent intent) {
		...
		if (intent.getData() != null) {
			...
			callForFunction(START_FRAGMENT, intent);
		}
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (TextUtils.isEmpty(BCBSharedPrefUtils.getUserID(this))
				&& ((BarcampBangalore) getApplicationContext()).isLoginShown == false) {
			startActivity(new Intent(this, LoginActivity.class));
			((BarcampBangalore) getApplicationContext()).isLoginShown = true;
			Intent newIntent = new Intent(this,
					UpdatesFetcherIntentService.class);
...
//			fragment.setCancelable(false);
			...
			try {
				FragmentManager frManager = getSupportFragmentManager();
				Class<?> clazz = (Class<?>) params
						.getSerializableExtra(BCBFragmentUtils.CLASS_NAME);
